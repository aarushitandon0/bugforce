...
export function ChallengeCard({
  ...
  return (
    ...
    <article
      className={`group relative flex h-full flex-col border p-4 transition-colors duration-[120ms] hover:border-dim focus-within:border-dim ${
        marker === "next" ? "border-dim" : "border-line"
      }`}
    >
      ...
      <p className="mt-4 flex items-center justify-between gap-4 text-[11px] text-dim">
        ...
        {marker === "solved" ? (
          ...
          <span className={`transition-colors duration-[120ms] group-hover:text-text ${marker === "next" ? "text-text" : "text-dim"}`}>
